...
def get_pil_font (font_path_or_name :str |None ,size :int ,font_index :int =0 )->ImageFont .FreeTypeFont |ImageFont .ImageFont |None :
    ...
    try :
        if actual_font_path and (os .path .exists (actual_font_path )or not os .path .isabs (actual_font_path )):
...
line_or_col_spacing_px :int =0 
    ...
    if orientation =="horizontal":
        ...
        while current_char_idx <len (text ):
            ...
            if char_val =='\n':
                if current_line_text :
                    ...
                    current_w =draw .textlength (current_line_text ,font =font )+(char_spacing_px *(len (current_line_text )-1 )if len (current_line_text )>1 and char_spacing_px !=0 else 0 )
        ...
        if not output_segments and text :
            ...
            max_line_width_achieved =draw .textlength (text ,font =font )+(char_spacing_px *(len (text )-1 )if len (text )>1 and char_spacing_px !=0 else 0 )
        total_dim_primary =0 
        if output_segments :
        ...
        if col_width_metric_for_total ==0 :col_width_metric_for_total =default_font_size 
