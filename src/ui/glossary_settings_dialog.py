...
QLineEdit ,QPushButton ,QFileDialog ,QWidget ,QSpacerItem ,
...
class GlossarySettingsDialog (QDialog ):
    ...
    def _init_ui (self ):
        ...
        glossary_file_actions_layout .addSpacerItem (QSpacerItem (40 ,20 ,QSizePolicy .Policy .Expanding ,QSizePolicy .Policy .Minimum ))
    ...
    def _populate_bulk_text_from_list (self ):
        ...
        for source ,target ,original_line in self .glossary_terms :
            if "#"in original_line and original_line .startswith (source )and "->"in original_line :
    ...
    def _add_glossary_term (self ):
        ...
        for s ,_ ,_ in self .glossary_terms :
            if s ==source :
                QMessageBox .warning (self ,"重复术语",f"原文 '{source}' 已存在于术语表中。")
                return 
    ...
    def _delete_selected_glossary_term (self ):
        ...
        if reply ==QMessageBox .StandardButton .No :
            return 
        indices_to_remove_from_terms_list =sorted ([item .data (Qt .ItemDataRole .UserRole )for item in selected_items ],reverse =True )
    ...
    def _import_glossary_from_file (self ):
        ...
        try :
            ...
            reply =QMessageBox .question (self ,"导入模式",
            "追加到当前术语列表还是替换整个列表？\n(Yes=追加, No=替换)",
            ...
            self ._rebuild_list_widget_from_terms ()
            self .config_manager .set ('UI','last_glossary_dir',os .path .dirname (file_path ))
    ...
    def _export_glossary_to_file (self ):
        if not self .glossary_terms :
            ...
            return 
        start_dir =self .config_manager .get ('UI','last_glossary_dir',os .path .expanduser ("~"))
        file_path ,_ =QFileDialog .getSaveFileName (self ,"导出术语文件",
    ...
    def _connect_signals (self ):
        ...
        self .glossary_delete_selected_button .clicked .connect (self ._delete_selected_glossary_term )
        self .glossary_parse_from_text_button .clicked .connect (self ._parse_and_load_from_bulk_text )
