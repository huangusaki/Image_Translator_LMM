...
QDoubleSpinBox ,QToolButton ,QMenu ,QColorDialog ,QSpinBox 
)
...
class InteractiveLabel (QWidget ):
    ...
    def reload_style_configs (self ):
        ...
        for block_item in self .processed_blocks :
            ...
            if not hasattr (block_item ,'main_color'):block_item .main_color =None 
            if not hasattr (block_item ,'outline_color'):block_item .outline_color =None 
            if not hasattr (block_item ,'background_color'):block_item .background_color =None 
            if not hasattr (block_item ,'outline_thickness'):block_item .outline_thickness =None 
    ...
    def _get_block_visual_hash (self ,block :ProcessedBlock )->int :
        main_color_to_hash =block .main_color if hasattr (block ,'main_color')and block .main_color is not None else self ._text_main_color_pil 
        outline_color_to_hash =block .outline_color if hasattr (block ,'outline_color')and block .outline_color is not None else self ._text_outline_color_pil 
        bg_color_to_hash =block .background_color if hasattr (block ,'background_color')and block .background_color is not None else self ._text_bg_color_pil 
        outline_thickness_to_hash =block .outline_thickness if hasattr (block ,'outline_thickness')and block .outline_thickness is not None else self ._outline_thickness 
        relevant_attrs =(
    ...
    def _get_or_render_block_qpixmap (self ,block :ProcessedBlock )->QPixmap |None :
        ...
        main_color =block .main_color if hasattr (block ,'main_color')and block .main_color is not None else self ._text_main_color_pil 
        outline_color =block .outline_color if hasattr (block ,'outline_color')and block .outline_color is not None else self ._text_outline_color_pil 
        bg_color =block .background_color if hasattr (block ,'background_color')and block .background_color is not None else self ._text_bg_color_pil 
        thickness =block .outline_thickness if hasattr (block ,'outline_thickness')and block .outline_thickness is not None else self ._outline_thickness 
    ...
    def set_processed_blocks (self ,blocks :list [ProcessedBlock ]):
        ...
        for i ,block in enumerate (self .processed_blocks ):
            ...
            if not hasattr (block ,'main_color'):block .main_color =None 
            if not hasattr (block ,'outline_color'):block .outline_color =None 
            if not hasattr (block ,'background_color'):block .background_color =None 
            if not hasattr (block ,'outline_thickness'):block .outline_thickness =None 
            fixed_font_size_override =self .config_manager .getint ('UI','fixed_font_size',0 )
            if fixed_font_size_override >0 :
    ...
    def paintEvent (self ,event ):
        ...
        for block in self .processed_blocks :
            ...
            if block ==self .selected_block :
                ...
                effective_display_scale_x =bg_img_to_display_scale_x if bg_img_to_display_scale_x >0.001 else 1.0 
                effective_display_scale_y =bg_img_to_display_scale_y if bg_img_to_display_scale_y >0.001 else 1.0 
                effective_display_scale_avg =(effective_display_scale_x +effective_display_scale_y )/2.0 
                ...
                rot_handle_offset_local_y =float (ROTATION_HANDLE_OFFSET )/effective_display_scale_y 
    ...
    def _get_handle_rects_for_block (self ,block :ProcessedBlock )->tuple [list [QRectF ],QRectF ]:
        ...
        bg_img_to_display_scale_x ,bg_img_to_display_scale_y =self ._get_bg_fit_scale_factors ()
        unscale_x =1.0 /bg_img_to_display_scale_x if bg_img_to_display_scale_x !=0 else 1.0 
        ...
        effective_scale_y =bg_img_to_display_scale_y if bg_img_to_display_scale_y >0.001 else 1.0 
    ...
    def _get_bg_fit_scale_factors (self )->tuple [float ,float ]:
        ...
        self .scaled_background_pixmap .width ()>0 and self .scaled_background_pixmap .height ()>0 ):
            scale_x =self .scaled_background_pixmap .width ()/self .background_pixmap .width ()
            scale_y =self .scaled_background_pixmap .height ()/self .background_pixmap .height ()
    ...
    def contextMenuEvent (self ,event :QContextMenuEvent ):
        ...
        if block_under_mouse :
            ...
            current_orientation =getattr (self .selected_block ,'orientation','horizontal')
            ...
            if action ==edit_action :
                ...
                self .set_selected_block (None )
                self .update ()
            elif action in [set_horiz_action ,set_vert_rtl_action ,set_vert_ltr_action ]and self .selected_block :
                new_orientation =self .selected_block .orientation 
                if action ==set_horiz_action :new_orientation ="horizontal"
                elif action ==set_vert_rtl_action :new_orientation ="vertical_rtl"
                elif action ==set_vert_ltr_action :new_orientation ="vertical_ltr"
                if self .selected_block .orientation !=new_orientation :
                    self .selected_block .orientation =new_orientation 
                    self ._invalidate_block_cache (self .selected_block )
                    self .block_modified_signal .emit (self .selected_block )
            elif action in [set_align_left_action ,set_align_center_action ,set_align_right_action ]and self .selected_block :
                new_align =self .selected_block .text_align 
                if action ==set_align_left_action :new_align ="left"
                elif action ==set_align_center_action :new_align ="center"
                elif action ==set_align_right_action :new_align ="right"
    ...
    def _add_new_text_block (self ,pos_widget :QPointF ):
        ...
        default_font_size_px =self .font_size_mapping .get (default_font_size_category ,22 )
        fixed_font_size_override =self .config_manager .getint ('UI','fixed_font_size',0 )
        ...
        new_block =ProcessedBlock (id =new_id ,original_text ="",translated_text ="新文本框",
        bbox =new_bbox ,orientation ="horizontal",
        font_size_pixels =default_font_size_px ,
        angle =0.0 ,text_align ="left",
...
class BatchTranslationWorker (QThread ):
    ...
    def run (self ):
            ...
            if result_tuple :
                ...
                if final_drawn_pil_image :
                    ...
                    try :
                        ...
                        elif output_filename .lower ().endswith (".bmp"):save_format ="BMP"
                        if save_format =="JPEG"and final_drawn_pil_image .mode =='RGBA':
                            bg =Image .new ("RGB",final_drawn_pil_image .size ,(255 ,255 ,255 ))
                            bg .paste (final_drawn_pil_image ,mask =final_drawn_pil_image .split ()[3 ])
                    ...
                    err_msg =f"绘制文本块失败: {current_file_basename}"+(f" (原始处理错误: {last_proc_error})"if last_proc_error else "")
        ...
        final_progress =int (((total_attempted )/total_files )*100 )if total_files >0 else 100 
...
class MainWindow (QMainWindow ):
    ...
    def _check_dependencies_on_startup (self ):
        ...
        missing =[name for name ,available in deps .items ()if not available and name =="Pillow"]
        if missing :
             QMessageBox .critical (self ,"依赖缺失",f"{', '.join(missing)} 库未安装！部分功能将无法使用。")
    def _apply_initial_settings (self ):
        ...
        if icon_path and os .path .exists (icon_path ):
            if self ._apply_window_icon (icon_path ):
             ...
             self .config_manager .set ('UI','window_icon_path','')
        if hasattr (self ,'interactive_translate_area')and self .interactive_translate_area :
    ...
    def _create_central_widget (self ):
        ...
        self .block_font_size_spin .setRange (5 ,200 );self .block_font_size_spin .setSingleStep (1 )
        self .block_font_size_spin .setDecimals (0 );self .block_font_size_spin .setButtonSymbols (QDoubleSpinBox .ButtonSymbols .PlusMinus )
        ...
        self .block_angle_spin .setRange (-360 ,360 );self .block_angle_spin .setSingleStep (1 )
        self .block_angle_spin .setDecimals (1 );self .block_angle_spin .setButtonSymbols (QDoubleSpinBox .ButtonSymbols .PlusMinus )
        ...
        button_layout .addWidget (self .translate_button );button_layout .addSpacing (20 );
    ...
    def _create_color_button (self ,initial_color :QColor =Qt .GlobalColor .gray )->QPushButton :
        button =QPushButton ()
    ...
    def _set_button_color (self ,button :QPushButton ,color :QColor ):
        if not isinstance (color ,QColor ):
             try :
                 ...
                 color =QColor (Qt .GlobalColor .gray )
        button .setStyleSheet (f"background-color: {color.name(QColor.NameFormat.HexArgb)}; border: 1px solid gray;")
        button .setToolTip (f"当前颜色: {color.name(QColor.NameFormat.HexArgb)}")
        button .setProperty ("currentColorTuple",(color .red (),color .green (),color .blue (),color .alpha ()))
    def _parse_color_str (self ,color_str :str ,default_color_tuple :tuple )->tuple :
        try :
            parts =list (map (int ,color_str .split (',')))
            if len (parts )==3 :return (parts [0 ],parts [1 ],parts [2 ],255 )
    ...
    def _update_block_controls (self ,block :ProcessedBlock |None ):
        ...
        self .block_text_edit_proxy .setText (""if not is_block_selected else (block .translated_text [:50 ]+"..."if len (block .translated_text )>50 else block .translated_text ))
        ...
        self .block_font_size_spin .setValue (block .font_size_pixels if is_block_selected else self .interactive_translate_area .font_size_mapping .get ('medium',22 ))
        self .block_angle_spin .setValue (block .angle if is_block_selected else 0.0 )
        default_thickness =self .interactive_translate_area ._outline_thickness if hasattr (self .interactive_translate_area ,'_outline_thickness')else 2 
        self .outline_thickness_spin .setValue (block .outline_thickness if is_block_selected and hasattr (block ,'outline_thickness')and block .outline_thickness is not None else default_thickness )
        default_main_color =self .interactive_translate_area ._text_main_color_pil if hasattr (self .interactive_translate_area ,'_text_main_color_pil')else (255 ,255 ,255 ,255 )
        main_color_tuple =block .main_color if is_block_selected and hasattr (block ,'main_color')and block .main_color is not None else default_main_color 
        self ._set_button_color (self .main_color_button ,QColor (*main_color_tuple ))
        default_outline_color =self .interactive_translate_area ._text_outline_color_pil if hasattr (self .interactive_translate_area ,'_text_outline_color_pil')else (0 ,0 ,0 ,255 )
        outline_color_tuple =block .outline_color if is_block_selected and hasattr (block ,'outline_color')and block .outline_color is not None else default_outline_color 
        self ._set_button_color (self .outline_color_button ,QColor (*outline_color_tuple ))
        default_bg_color =self .interactive_translate_area ._text_bg_color_pil if hasattr (self .interactive_translate_area ,'_text_bg_color_pil')else (0 ,0 ,0 ,128 )
        bg_color_tuple =block .background_color if is_block_selected and hasattr (block ,'background_color')and block .background_color is not None else default_bg_color 
    ...
    def _on_selected_block_spinbox_changed (self ):
        if self .interactive_translate_area .selected_block :
            ...
            current_thickness =s_block .outline_thickness if hasattr (s_block ,'outline_thickness')else None 
    ...
    def _on_pick_color (self ):
        ...
        if sender_button ==self .main_color_button :
            ...
            current_color_tuple =s_block .main_color if hasattr (s_block ,'main_color')and s_block .main_color else self .interactive_translate_area ._text_main_color_pil 
        elif sender_button ==self .outline_color_button :
            attribute_name ='outline_color'
            current_color_tuple =s_block .outline_color if hasattr (s_block ,'outline_color')and s_block .outline_color else self .interactive_translate_area ._text_outline_color_pil 
        elif sender_button ==self .background_color_button :
             attribute_name ='background_color'
             current_color_tuple =s_block .background_color if hasattr (s_block ,'background_color')and s_block .background_color else self .interactive_translate_area ._text_bg_color_pil 
        else :
            return 
        initial_qcolor =QColor (*current_color_tuple )
        new_qcolor =QColorDialog .getColor (initial_qcolor ,self ,"选择颜色",options =QColorDialog .ColorDialogOption .ShowAlphaChannel )
        if new_qcolor .isValid ():
            new_color_tuple =(new_qcolor .red (),new_qcolor .green (),new_qcolor .blue (),new_qcolor .alpha ())
    ...
    def _connect_signals (self ):
        ...
        self .block_font_size_spin .valueChanged .connect (self ._on_selected_block_spinbox_changed )
        self .block_angle_spin .valueChanged .connect (self ._on_selected_block_spinbox_changed )
        self .outline_thickness_spin .valueChanged .connect (self ._on_selected_block_spinbox_changed )
    ...
    def _on_load_image (self ):
        ...
        if file_path :
            if not PILLOW_AVAILABLE :
                QMessageBox .critical (self ,"依赖缺失","Pillow未安装，无法加载图片。");return 
            try :
    ...
    def _on_change_window_background (self ):
        ...
        if file_path :
            ...
            if bg_pixmap .isNull ():
                QMessageBox .warning (self ,"加载错误",f"无法加载背景图片: {file_path}");return 
    ...
    def _apply_window_background (self ,bg_pixmap :QPixmap )->bool :
        if not bg_pixmap or bg_pixmap .isNull ():return False 
        try :
            opacity =self .config_manager .getfloat ('UI','background_opacity',0.15 )
            target_size =self .size ()
            if target_size .width ()<=0 or target_size .height ()<=0 :return False 
            ...
            palette .setBrush (QPalette .ColorRole .Window ,QBrush (QPixmap .fromImage (temp_image )))
        ...
        if file_path :
            if self ._apply_window_icon (file_path ):
                ...
                self .current_icon_path =file_path 
                 ...
                 QMessageBox .warning (self ,"图标错误",f"无法加载或应用图标: {file_path}")
    def _apply_window_icon (self ,icon_path :str )->bool :
        ...
        if PILLOW_AVAILABLE :
            try :
                ...
                print (f"Pillow icon processing for rounding failed: {pillow_err}. Trying direct QIcon.")
        if not applied :
    ...
    def _on_open_text_style_settings (self ):
        ...
        result =dialog .exec ()
        try :
            dialog .settings_applied .disconnect (self ._handle_text_style_settings_applied_live )
        except TypeError :
            pass 
    ...
    def _on_translate_clicked (self ):
        if not self .current_image_path or not self .original_pil_for_display :
            self ._handle_error_message ("操作无效","请先加载图片。");return 
        if not PILLOW_AVAILABLE :
            self ._handle_error_message ("依赖缺失","Pillow 库未安装，无法处理图片。");return 
        if self .translation_worker and self .translation_worker .isRunning ():
             QMessageBox .information (self ,"处理中","当前翻译任务仍在进行中，请稍候。");return 
        if self .batch_worker and self .batch_worker .isRunning ():
             QMessageBox .information (self ,"处理中","当前有批量任务仍在进行中，请稍候。");return 
    ...
    error_message_str :str |None ):
        if self .current_image_path !=original_image_path_processed and not (self .batch_worker and self .batch_worker .isRunning ()):
            print (f"Ignoring stale translation result for: {original_image_path_processed}")
        ...
        if current_bg_for_interactive and not current_bg_for_interactive .isNull ():
            self .interactive_translate_area .set_background_image (current_bg_for_interactive )
        if self .text_detail_panel :self .text_detail_panel .clear_texts ()
        if error_message_str and not processed_blocks :
            self .interactive_translate_area .set_processed_blocks ([])
            self ._handle_error_message ("处理失败",f"处理图片时出错:\n{error_message_str}",
            informative_text =f"图片路径: {original_image_path_processed}",
            show_settings_option =("API"in error_message_str or "Key"in error_message_str or "认证"in error_message_str ))
            ...
            if processed_blocks :
                ...
                if error_message_str :
                    QMessageBox .warning (self ,"处理警告",f"处理完成，但出现以下问题:\n{error_message_str}")
            else :
                self .download_button .setEnabled (False )
                no_text_msg ="完成，未检测到文本。"
                final_status =no_text_msg if not error_message_str else f"状态: {error_message_str.splitlines()[0]}"
                self .status_label .setText (final_status )
                if error_message_str :
                     QMessageBox .information (self ,"处理信息",f"处理完成但报告:\n{error_message_str}")
                self ._update_block_controls (None )
        else :
             self .interactive_translate_area .set_processed_blocks ([])
             ...
             self .download_button .setEnabled (False )
             if error_message_str :
                 self ._handle_error_message ("处理异常",f"处理返回意外结果。\n错误信息: {error_message_str}",informative_text =f"图片: {original_image_path_processed}")
        self ._restore_ui_after_processing ()
        self .translation_worker =None 
        self .setWindowTitle ("Image Translator")
    ...
    def _on_download_clicked (self ):
        if not self .interactive_translate_area .background_pixmap :
             QMessageBox .warning (self ,"导出错误","没有背景图像可导出。");return 
        if not self .interactive_translate_area .processed_blocks :
             reply =QMessageBox .question (self ,"导出确认","当前没有文本块。是否要导出原始背景图片？",
             QMessageBox .StandardButton .Yes |QMessageBox .StandardButton .No ,
             QMessageBox .StandardButton .No )
             if reply ==QMessageBox .StandardButton .No :
                 return 
             bg_qimage =self .interactive_translate_area .background_pixmap .toImage ().convertToFormat (QImage .Format .Format_RGBA8888 )
             if bg_qimage .isNull ():
                 QMessageBox .warning (self ,"导出错误","无法获取背景图像数据。");return 
             buffer =QBuffer ()
             ...
             try :
                 ...
                 QMessageBox .warning (self ,"导出错误",f"转换背景图像时出错: {e}");return 
             finally :
                buffer .close ()
             if final_pil_to_save .mode !='RGBA':final_pil_to_save =final_pil_to_save .convert ('RGBA')
        else :
            final_pil_to_save =self .interactive_translate_area .get_current_render_as_pil_image ()
            if not final_pil_to_save :
                QMessageBox .warning (self ,"导出错误","无法生成最终的渲染图像。");return 
        ...
        save_path ,selected_filter =QFileDialog .getSaveFileName (self ,"保存翻译后的图片",
        os .path .join (start_dir ,default_name ),
        save_filter )
        if save_path :
            format_map ={"PNG 图片 (*.png)":"PNG",
            ...
            try :
                if img_format =="JPEG"and final_pil_to_save .mode =='RGBA':
                    print ("Converting RGBA to RGB for JPEG export with white background.")
                    rgb_image =Image .new ("RGB",final_pil_to_save .size ,(255 ,255 ,255 ))
                    rgb_image .paste (final_pil_to_save ,mask =final_pil_to_save .split ()[3 ])
                ...
                QMessageBox .warning (self ,"保存错误",f"无法保存图片至:\n{save_path}\n错误: {e}")
    @pyqtSlot ()
    def _on_load_batch_images (self ):
        if (self .translation_worker and self .translation_worker .isRunning ())or (self .batch_worker and self .batch_worker .isRunning ()):
            QMessageBox .warning (self ,"操作繁忙","当前已有任务在进行中，请等待完成后再开始批量处理。");return 
        if not PILLOW_AVAILABLE :
            self ._handle_error_message ("依赖缺失","Pillow 库未安装，无法处理图片。");return 
        img_filter ="图片 (*.png *.jpg *.jpeg *.bmp *.gif *.tif *.tiff);;所有文件 (*)"
        start_dir =self .config_manager .get ('UI','last_image_dir',os .path .expanduser ("~"))
        file_paths ,_ =QFileDialog .getOpenFileNames (self ,"选择需要批量翻译的图片",start_dir ,img_filter )
        if not file_paths :return 
        output_dir =QFileDialog .getExistingDirectory (self ,"选择翻译结果保存目录",start_dir )
    ...
    def _on_batch_finished (self ,processed_count :int ,error_count :int ,duration :float ,cancelled :bool ):
        self .setWindowTitle ("Image Translator")
        output_dir_msg =getattr (self .batch_worker ,'output_dir','N/A')if self .batch_worker else 'N/A'
        ...
        f"总耗时: {duration:.2f} 秒\n\n"
        f"结果已保存至:\n{output_dir_msg}")
        self .status_label .setText (f"状态: {status_prefix}。成功: {processed_count}, 失败/错误: {error_count}")
