...
class InteractiveLabel(QWidget):
    ...
    def contextMenuEvent(self, event: QContextMenuEvent):
        ...
        if block_under_mouse:
            ...
            delete_action = menu.addAction("删除此块")
            menu.addSeparator()
            ...
            set_align_left_action = align_menu.addAction("左对齐 / (竖排时)顶对齐字符")
            set_align_center_action = align_menu.addAction("居中对齐 / (竖排时)居中字符")
            set_align_right_action = align_menu.addAction("右对齐 / (竖排时)底对齐字符")
...
class MainWindow(QMainWindow):
    ...
    def _create_central_widget(self):
        ...
        self.original_preview_area.setMinimumSize(300,300); self.original_preview_area.setWordWrap(True)
        splitter.addWidget(self.original_preview_area)
        self.interactive_translate_area = InteractiveLabel(self.config_manager, self)
        splitter.addWidget(self.interactive_translate_area)
        splitter.setStretchFactor(0,1); splitter.setStretchFactor(1,2)
        main_layout.addWidget(splitter,1); self.splitter = splitter
        self.block_controls_widget = QWidget(); block_controls_layout = QHBoxLayout(self.block_controls_widget)
        block_controls_layout.setContentsMargins(0,0,0,0); block_controls_layout.addWidget(QLabel("选中块:"))
    ...
    def _update_block_controls_from_interaction(self, modified_block: ProcessedBlock):
        if self.interactive_translate_area.selected_block == modified_block: self._update_block_controls(modified_block)
    @pyqtSlot()
    ...
    def _on_translation_finished(self, original_pil_image_from_worker: Image.Image | None,
        ...
        if error_message_str and not processed_blocks : 
            ...
            if processed_blocks: 
                ...
                if not self.interactive_translate_area.selected_block: self.interactive_translate_area.set_selected_block(processed_blocks[0])
