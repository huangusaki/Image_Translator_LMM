...
class TextStyleSettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.apply_button = QPushButton("应用")  # New Apply button
        self.save_button = QPushButton("保存")
        self.cancel_button = QPushButton("取消")
        button_layout.addWidget(self.apply_button) # Add Apply button to layout
    ...
    def _load_settings(self):
        ...
        self.h_text_char_spacing_edit.setText(self.config_manager.get('UI', 'h_text_char_spacing_px', fallback='0'))
    ...
    def _connect_signals(self):
        ...
        self.apply_button.clicked.connect(self.on_apply) # Connect Apply button
    ...
    def _perform_validation(self):
        ...
        for edit_field, name in [
            ...
            if spacing_str: # Only validate if not empty, empty will default to '0' in _save_settings
                try:
                    int(spacing_str)
                except ValueError:
        ...
        for edit_field, name in [
            ...
            if not color_str: # Color strings should not be empty
                 QMessageBox.warning(self, "输入错误", f"{name} 不能为空。")
                 edit_field.setFocus()
                 return False
            parts = color_str.split(',')
            if not (len(parts) == 3 or len(parts) == 4) or \
               not all(p.strip().isdigit() for p in parts):
            ...
            for part_str in parts:
                try:
                    ...
                    if not (0 <= val <= 255):
                        QMessageBox.warning(self, "输入错误", f"{name} 中的颜色值必须在 0 到 255 之间。")
                        edit_field.setFocus()
                        return False
        ...
        if not thickness_str.isdigit(): # isdigit also covers empty string case
            QMessageBox.warning(self, "输入错误", "文本描边厚度必须是一个非负整数数字。")
            self.text_outline_thickness_edit.setFocus()
            return False
        # isdigit() ensures it's a non-negative integer string.
    ...
    def on_apply(self):
        """
        Validates and saves settings without closing the dialog.
        """
        if self._perform_validation():
            if self._save_settings():
                QMessageBox.information(self, "应用成功", "设置已应用。")
                # Here you might want to emit a signal if other parts of your application
                # need to react to applied settings immediately.
            # else:
                # Handle potential save failure if _save_settings could return False
                # QMessageBox.critical(self, "错误", "保存设置失败。")

    @pyqtSlot()
    def on_save(self):
        """
        Validates, saves settings, and closes the dialog if successful.
        """
        if self._perform_validation():
            if self._save_settings():
                self.accept() # Closes the dialog with QDialog.Accepted result
            # else:
                # Handle potential save failure
                # QMessageBox.critical(self, "错误", "保存设置失败。")

# Example Usage (requires a mock config_manager)
if __name__ == '__main__':
    ...
    class MockConfigManager:
        def __init__(self):
            self.config = {'UI': {}}
            # Set some initial fallbacks to mimic real behavior for loading
